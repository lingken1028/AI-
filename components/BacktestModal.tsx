...
const BacktestModal: React.FC<BacktestModalProps> = ({ isOpen, onClose, symbol }) => {
  const [strategy, setStrategy] = useState<BacktestStrategy>(BacktestStrategy.MACD_GOLDEN_CROSS);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="bg-[#151c24] border border-gray-700 w-full max-w-2xl rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
        ...
        {/* Header */}
        <div className="p-6 border-b border-gray-800 flex items-center justify-between shrink-0 bg-gradient-to-r from-blue-900/10 to-transparent">
          <div>
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <div>
              <label className="block text-xs text-gray-400 font-bold uppercase mb-2">选择交易策略</label>
              <select 
                value={strategy} 
                onChange={(e) => setStrategy(e.target.value as BacktestStrategy)}
                className="w-full bg-[#0b1215] border border-gray-700 rounded-lg p-3 text-sm text-white focus:border-blue-500 focus:outline-none"
              >
            ...
            <div>
              <label className="block text-xs text-gray-400 font-bold uppercase mb-2">回测时间范围</label>
              <select 
                value={period} 
          ...
          {result && (
            <div className="mt-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="bg-[#0b1215]/50 border border-gray-800 rounded-xl p-5 mb-4 space-y-4">
                <div>
                   <h4 className="text-xs text-gray-500 uppercase font-bold mb-2">资金曲线描述</h4>
...
const ResultCard = ({ label, value, icon, highlight }: any) => (
  <div className={`bg-[#0b1215] p-4 rounded-xl border ${highlight ? 'border-green-500/30 bg-green-500/5' : 'border-gray-800'} flex flex-col items-center text-center`}>
